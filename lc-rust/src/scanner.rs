...
mod tests {
    ...
    fn it_scans_mutiple_lines() {
        assert_eq!(
            scan("  (  \n  )  ").unwrap(),
            vec![SrcToken(BracketOpen, pos(1, 3, 2)), SrcToken(BracketClose, pos(2, 3, 8)), SrcToken(EOF, pos(2, 6, 11))],
        );
    }
    ...
    fn it_scans_simple_function() {
        assert_eq!(
            scan("λa.b").unwrap(),
            vec![SrcToken(Lambda, pos(1, 1, 0)), SrcToken(Symbol("a".to_string()), pos(1, 2, 1)),
                SrcToken(Dot, pos(1, 3, 2)), SrcToken(Symbol("b".to_string()), pos(1, 4, 3)),
                SrcToken(EOF, pos(1, 5, 4))
            ],
        );
    }
    ...
    fn it_scans_camel_case_symbol() {
        assert_eq!(
            ...
            vec![SrcToken(Symbol("camelCase".to_string()), pos(1, 1, 0)), SrcToken(EOF, pos(1, 10, 9))],
        );
    }
    ...
    fn it_scans_integers() {
        assert_eq!(
            scan("123456").unwrap(),
            vec![SrcToken(Integer(123456), pos(1, 1, 0)), SrcToken(EOF, pos(1, 7, 6))],
        );
    }
    ...
    fn it_scans_let_expression() {
        assert_eq!(
            ...
            vec![
                SrcToken(Let, pos(1, 1, 0)),
                SrcToken(Symbol("f".to_string()), pos(1, 5, 4)),
                SrcToken(Equals, pos(1, 7, 6)),
                SrcToken(Symbol("b".to_string()), pos(1, 9, 8)),
                SrcToken(In, pos(1, 11, 10)),
                SrcToken(Symbol("e".to_string()), pos(1, 14, 13)),
                SrcToken(EOF, pos(1, 15, 14)),
            ],
        );
    }
    ...
    fn pos(line: u16, col: u16, index: u16) -> SrcPosition {
        SrcPosition { line, col, index }
    }
}
...
pub fn scan(src: &str) -> Result<Vec<SrcToken>, Error> {
    ...
    loop {
        ...
        if INTEGER.is_match(&c.to_string()) {
            let pos = SrcPosition { line, col, index: index - 1  };
    ...
    v.push(SrcToken(Token::EOF, SrcPosition { line, col: col + 1, index: index }));
    Ok(v)
}
